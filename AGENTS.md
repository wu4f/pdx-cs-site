...
- **`toc.py`** — `inject_toc(body_html, threshold=7, url_path="")` injects a clickable table of contents after the first `</h1>` when the page has ≥ 7 h2/h3/h4 headings. Uses the `id` attributes already present on Google Docs-exported headings as anchor targets; links are prefixed with `url_path` so they resolve correctly under `<base href="/">`. Hierarchical numbering (`1.`, `2.1.`, `3.2.4.`) is handled entirely by CSS counters in `templates/base.html`. All content headings carry `scroll-margin-top: 160px` to keep them clear of the sticky two-row header when navigating via anchor.
