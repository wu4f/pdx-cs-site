...
# Regenerate sitemap.xml, robots.txt, llms.txt, and llms-full.txt from an existing sections.json
# (no Google API calls)
...
Google Docs ──► sources/ (splitters) ──► []Section ──► categorize.py
                                                    ...
                                                    ──► llms.py           → build/site/llms.txt, llms-full.txt
...
|---|---|
| `tabs` | one Google Doc tab → one Section; uses the Docs API and the same renderer as `whole_splitter` (`_walk_elements`, `_resolve_internal_links`), avoiding the HTML-export endpoint entirely — no inter-tab sleep, no 429 exposure from tab iteration. Style is always `""`. Tables gain proper `<thead>`/`<th>` output. |
| `whole` | entire doc → one Section; renders the Docs API structural elements with a minimal in-house renderer in `whole_splitter.py` (paragraphs, text runs, headings, lists, tables, horizontal rules; images/inline objects are skipped). Also fetches one HTML export per tab, but only to read list glyphs out of its stylesheet — see below |

Export throttling: the HTML export endpoint (`gdocs.export_tab_html`) is not the Docs API and has its own, much tighter, undocumented quota — two full builds back to back are enough to start drawing 429s, and the block clears in minutes rather than seconds. `tab_splitter` no longer calls this endpoint at all (it uses the Docs API renderer); the throttling discussion below applies only to `whole_splitter`'s `_list_glyphs()` calls (one per tab, only when the doc has lists). So 429/408 back off from 15 s (15/30/60/120…, capped at 120 s, ≈ 8 minutes over 7 attempts) while timeouts, connection errors, and 5xx retry from 1 s; a `Retry-After` header wins over the ladder, capped at 300 s so a build can't stall for an hour. 401/403/404 fail immediately — waiting can't fix them. Errors are raised via `_export_error()`, which puts the status, reason, and URL in the message; plain `requests.HTTPError(response=r)` stringifies to nothing and makes a failed build unreadable.
...
Multi-paragraph list items: the way a Docs author writes one is to press Enter inside a bullet, clear the bullet, and leave the text at the item's indent (indenting one step further for a sub-list) — Docs then lays that content out hanging under the bullet. Two things have to follow from the indent rather than from the API's list fields. (1) `_is_continuation()` keeps a list run open across any non-heading paragraph indented at least as far as the run's outermost bullet; those paragraphs enter the run as `(None, indent, html)` entries and `_render_list()` emits them *inside* the enclosing `<li>` (unwinding out of any open sub-list first, so a `<p>` never becomes a direct child of a `<ul>`). Closing the list at the first non-bullet paragraph instead would drop the rest of the item to flush-left and read as if the list had ended. A blank indented paragraph is Docs' paragraph spacing, not content, so it is skipped rather than emitted as an empty `<p>` inside the item. (2) `_depths()` derives HTML nesting depth from `paragraphStyle.indentStart` (via `_indent()`, which falls back to the list's nesting-level definition), because `nestingLevel` counts only *within* one list: an author who indents a sub-list rather than pressing Tab inside the parent gets a brand-new `listId` whose items are all at `nestingLevel 0`, which would otherwise render as siblings of the parent bullets. Glyph lookup, `_Ctx.counts`, and `start=` still key off the Docs `nestingLevel` — the export stylesheet is indexed by it — so only the nesting follows the indent. Soft breaks (Shift+Enter) remain the way to get a plain line break inside an item; they need none of this.
...
Table rendering in `whole_splitter.py` emits `<table class="doc-table">` (or `<table class="doc-table pinned-header">`) with `<thead>`/`<tbody>`. Three things the Docs API makes non-obvious: (1) cells merged away by a neighbour's `rowSpan`/`columnSpan` still come back as empty placeholder cells at their grid position, so the renderer tracks covered `(row, col)` pairs and skips them — otherwise every merged row grows extra cells; (2) `tableRowStyle.tableHeader` is only set when the author pins a header row, which most docs never do, so the leading row is treated as the header by default. Only a *leading* run of pinned rows can become `<thead>`, since `<thead>` must precede `<tbody>`; (3) whether any `tableRowStyle.tableHeader` rows were found determines the `pinned-header` class — tables with `pinned-header` get the no-more-tables mobile stacking layout (see below), while tables without it (e.g. MECOP comparison matrices) continue to scroll horizontally on mobile. The `doc-table` class exists so `templates/base.html` can relax the `white-space: nowrap` header rule that the short-headed schedule tables rely on.

`whole_splitter.py` also renders horizontal rules: a paragraph whose `elements` list contains a `horizontalRule` element emits `<hr>` (flushing any open list run first) instead of a paragraph.

Mobile table behaviour (`templates/base.html`): `html { overflow-x: hidden }` clips horizontal overflow at the root element (clipping on `.page-layout` instead caused the sticky nav to be affected). Tables with `class="doc-table pinned-header"` stack as labelled cards on screens ≤ 600 px via a "no-more-tables" CSS block that sets `display: block` on all table/row/cell elements and hides the `<thead>` row off-screen with `position: absolute; top: -9999px`. Inline JS reads each `<thead th>` text and stamps the matching column's `<td>` cells with a `data-title` attribute; a CSS `::before` pseudo-element on `td` uses `content: attr(data-title)` to render the column label in the left 38% of the stacked cell. Tables without `pinned-header` are unaffected by these rules.
...
- **`landing.py`** — Renders `templates/landing.html` → `build/site/index.html`. Exports `build_nav_groups(sections, exclude_ids)` (groups sections by category in `CATEGORY_ORDER` order, used by all pages; **within** a category, sections keep source order — the Google Doc's tab order, then whole-docs in `content.yaml` `docs[]` order — so a page's position in a dropdown or landing column is changed by dragging its tab in the Doc, not in code), `meta_description(text)` (truncates plain text at last word boundary ≤160 chars), and `_site_base_url()` (reads `$SITE_BASE_URL`).
...
Per-term tables are emitted as `<table class="doc-table pinned-header">`, opting into the same "no-more-tables" mobile stacking layout used by tab_splitter tables (see the Sources / splitters section above): on screens ≤ 600 px each row renders as a labelled card via `base.html`'s inline JS + CSS, rather than horizontally scrolling.
...
### llms.txt (`cspdx/llms.py`)

Writes the [llms.txt](https://llmstxt.org/) pair for AI agents. `generate_llms_txt()` writes `build/site/llms.txt`: an H1, the landing description as a blockquote, then one `- [Title](url): description` line per active section grouped under `build_nav_groups()` categories (so order matches the nav), plus an `## Optional` block linking `llms-full.txt` and the sitemap. Descriptions are `meta_description()` of `section.text` with the leading heading text stripped, since `.text` begins with the page's own H1. `generate_llms_full_txt()` writes `build/site/llms-full.txt`: every active page as Markdown, each introduced by `---` and a `URL:` line.

Bodies are converted from `section.html` by `html_to_markdown()`, not taken from `section.text`, which is one run-on line per page with no headings, lists, tables, or links. The converter only covers the renderer's own tag set (h1–h6, p, br, ul/ol/li, table, a, strong/em/u, hr); relative hrefs are made absolute against `$SITE_BASE_URL`, literal `*` in text is escaped (docs use `**` as footnote marks), edge whitespace is moved outside `**`/`[...]` markers, and `rowspan`/`colspan` cells repeat their text in each slot because GFM tables have no spans. The `course-schedules` section is the exception: its HTML is the placeholder tab, so its body is `section.text` in a code block — which means `apply_schedule_text()` must run first. Generated by `cspdx build` (after robots.txt), `cspdx render-sitemap`, and `cspdx render-schedule` (which changes the schedule text).
...
build/
  site/                          # fully generated; nginx serves this directory
    ...
    llms.txt                     # llmstxt.org index of active pages for AI agents
    llms-full.txt                # all active pages as one Markdown file
